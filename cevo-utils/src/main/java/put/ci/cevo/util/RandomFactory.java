...
public interface RandomFactory<T> {
	 ...
	 * @return a new object
	 */
	T create(RandomDataGenerator random);

	default T create(ThreadedContext context) {
		return create(context.getRandomForThread());
	}

	static <T> RandomFactory<T> from(Factory<T> factory) {
		return random -> factory.create();
	}

	/** Create random factory generating samples/lists of objects */
	static <T> RandomFactory<List<T>> sampleFrom(RandomFactory<T> factory, int sampleSize) {
