...
import put.ci.cevo.games.tetris.StateValueFunctionTetrisActionEvaluator;
...
import put.ci.cevo.rl.agent.algorithms.TDAfterstateValueLearning;
...
public class TDLTetrisBIExperiment implements Runnable, Experiment {
	...
	private static final Logger logger = Logger.getLogger(TDLTetrisBIExperiment.class);
	...
	public void run() {
		...
		for (double eps : epses) {
			for (double alpha : alphas) {
				...
				StateValueFunctionTetrisActionEvaluator moveEvaluator = new StateValueFunctionTetrisActionEvaluator(
						stateValueFunction);
				Agent<TetrisState, TetrisAction> agent = new ActionEvaluatingAgent<>(tetris, moveEvaluator);
