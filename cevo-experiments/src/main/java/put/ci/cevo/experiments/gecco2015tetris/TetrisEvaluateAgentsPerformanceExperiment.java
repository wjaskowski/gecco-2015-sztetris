...
public class TetrisEvaluateAgentsPerformanceExperiment implements Experiment {
	...
	public void run(String[] args) {
		...
		evaluateAgent(new GECCO2015BestBICEMSZTetrisAgent()::create,
				GECCO2015BestBICEMSZTetrisAgent.class.getSimpleName(), NUM_GAMES);
	}
	...
	private void saveAnimationForAgent(Agent<TetrisState, TetrisAction> agent, String dir, ThreadedContext context) {
		...
		double result = tetris.interact(agent, context.getRandomForThread(), transition -> {
			if (wasReward[0]) {
